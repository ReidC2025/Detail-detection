...
print(class(t(BottomSVD$v[,1:BottomRank])))
...
Rank1=(sum((cumsum(SVDBase$d)/sum(SVDBase$d)) <= 0.90))+2
...
```{r}
ReturnList2=FindDetail(ReturnList$ImageT, 0.90)
...
plot(ReturnList2$CompImageLR, axes=FALSE)
plot(ReturnList2$ImageL, axes=FALSE)

###Compression of bottom component
ComponentB=ReturnList$ImageB
SVDCB=svd(ComponentB)
Rank2=(sum((cumsum(SVDCB$d)/sum(SVDCB$d)) <= 0.90))+2
CRB=((Rank2*(640+94))/(640*94))
CompBottom <- SVDCB$u[,1:Rank2] %*% diag(SVDCB$d[1:Rank2]) %*% t(SVDCB$v[,1:Rank2])

#Final Reconstruction
FinalReconstruction=as.cimg(cbind(as.matrix(ReturnList2$CompImageLR), CompBottom))



FinalCRatio=(94/360)*CRB+(266/360)*0.08698308
FinalCRatio
...
```{r}
((56*(640+360))/(640*360))
CompBaseMatch <- SVDBase$u[,1:56] %*% diag(SVDBase$d[1:56]) %*% t(SVDBase$v[,1:56])
